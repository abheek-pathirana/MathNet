...
config = GPT2Config(
    ...
    n_positions=64,     # context#256 for the algebraic model
    n_ctx=64,           #256(if you are tuning this hyperparameter then make sure both this and "n_positions" have the same number) and make sure u edit the "chunk_size=64" to the same value as this.
...
for epoch in range(epochs):
    ...
    model.save_pretrained(f"SML_arithmetic_pt{epoch+1}")#add any name you want


    tokenizer.save_pretrained(f"SML_arithmetic_pt{epoch+1}")#add any name you want


print("\nSample generation after epoch", epoch+1)
print(generate_sample_text(model, tokenizer))
        
model.save_pretrained("MathNet_pretrain_m1")


tokenizer.save_pretrained("MathNet_pretrain_t1")
