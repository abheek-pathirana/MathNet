...
# === Paths ===
tokenizer_path = "MathNet/src/pre_training/algebraic/SML_algebraic_pt70" #the path might differ and the last to digits in the path may differ depending on the number of epochs trained
...
with open("MathNet/src/task-oriented pre-training/algebraic/algebraic_task_oriented_pt.txt", "r", encoding="utf-8") as f:
    raw_text = f.read()
...
def generate_sample_text(model, tokenizer, prompt="<start_prompt>231*3<end_prompt>", max_length=256):
    model.eval()
...
for epoch in range(epochs):
    ...
    save_dir = f"MathNet_albebraic_v2_e{epoch+1}"
...
model.save_pretrained("algebraic_gpt2/best")
tokenizer.save_pretrained("algebraic_gpt2/best")